...
class TrainDiffusionUnetImageWorkspace(BaseWorkspace):
    ...
    def __init__(self, cfg: OmegaConf, output_dir=None):
        ...
        # Handle obs_encoder.pretrained config if it exists
        obs_encoder_pretrained = getattr(cfg.policy, 'obs_encoder', {}).get('pretrained', False)
        if obs_encoder_pretrained:
            obs_encorder_lr *= 0.1
        ...
        with JsonLogger(log_path) as json_logger:
            for local_epoch_idx in range(cfg.training.num_epochs):
                    ...
                    if len(val_dataloader) > 0:
                        with torch.no_grad():
                            ...
                            if len(val_losses) > 0:
                                ...
                                print(f"[32m✅ Validation loss at epoch {self.epoch}: {val_loss:.4f}[0m")
                            else:
                                print(f"[33m⚠️ No validation losses collected at epoch {self.epoch}[0m")
                    else:
                        print(f"[33m⚠️ Validation dataloader is empty at epoch {self.epoch}[0m")
                        # Use train loss as fallback for checkpoint saving
                        if 'loss' in step_log:
                            step_log['val_loss'] = step_log['loss']
                            print(f"[36m📊 Using train_loss as val_loss fallback: {step_log['loss']:.4f}[0m")
                ...
                if (self.epoch % checkpoint_every) == 0 and accelerator.is_main_process:
                    ...
                    # If val_loss missing, use train_loss as fallback
                    if 'val_loss' not in metric_dict and 'train_loss' in metric_dict:
                        metric_dict['val_loss'] = metric_dict['train_loss']
                        print(f"⚠️ val_loss not available at epoch {self.epoch}, using train_loss as fallback")
